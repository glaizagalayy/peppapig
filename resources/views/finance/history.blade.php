...
<div class="container-fluid py-4">
    ...
    <div class="card shadow-sm border-0">
        <div class="card-header d-flex justify-content-between align-items-center">
            ...
            <button class="btn btn-sm btn-light" data-bs-toggle="modal" data-bs-target="#manageBatchModal" aria-label="Manage Batches">
                        ...
                        @foreach ($students->sortBy('last_name') as $student)
                            @if (!request('batch_year') || $student->batch_year == request('batch_year'))
                                <tr>
                                    ...
                                    <td>₱{{ number_format($student->batch->total_due ?? 0, 2) }}</td>
                                    <td>
                ...
                <div class="modal-body">
                    <form action="{{ route('finance.updateBatch') }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="batch_year" id="batch_year" class="form-select" required aria-label="Select batch year">
                                @foreach ($batches as $batch)
                        ...
                        <div class="mb-3">
                            <label for="total_due" class="form-label fw-semibold">Total Amount Due</label>
                            <input type="number" name="total_due" id="total_due" class="form-control" required aria-label="Total amount due">
                        </div>
